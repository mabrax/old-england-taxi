...
This dated continuation was run on isolated branch `codex/desktop-qualification-astra`, starting at reviewed `179814c`; its reviewed implementation and qualification are now integrated in canonical main at `b1a5d06`. No runtime, vehicle, compiler, prepared artifact or schema change was planned beyond the reviewed implementation already recorded here. The user explicitly selected **Linux Chrome, Linux Firefox, and macOS Microsoft Edge** during this task. Desktop Safari is removed from acceptance scope, superseding its historical gate. iPad acceptance remains deferred. The user has a Mac but explicitly deferred its testing and limited this task to Linux Chrome/Firefox. macOS Edge is not passed and is not a required execution step in this task.
...
The separate user-coordinated memory repeat now completes all 20 Chrome process/GPU snapshots and passes the fixed JS/lifecycle checks; see the [record](verification.md#memory-evidence-and-its-limits). The user explicitly accepted the documented Linux measurement method. Reviewing process-memory growth remains distinct from inventing new numeric thresholds.
