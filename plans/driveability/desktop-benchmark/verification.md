...
## Deferred allocation and GC follow-up

The user's final handoff defers further allocation/GC investigation to [issue #1 — Analyze driving-loop allocations and garbage-collection pauses](https://github.com/mabrax/old-england-taxi/issues/1), alongside next-phase planning. The issue is open and is not a new planning blocker or authorization for another profiling campaign. Existing desktop qualification gates remain open; iPad acceptance remains deferred.

The subsequent full-interval analysis of the unchanged displayed capture is retained locally under `.zone-cache/desktop-benchmark/full-interval-review-01/` (`report.md`, `analysis.json`, `main-thread.sql`). It covers the complete 119.973-second measured wall interval and 52,918 complete renderer-main-thread RunTask slices. The largest task is 7.623 ms and contains a 7.512 ms MajorGC event; this alone establishes neither a leak nor the cause of the later slowest game frames. Investigation of allocation sites, retained memory, instrumentation overhead and justified storage reuse belongs to the deferred issue. No new performance run or runtime change was made for this handoff.
