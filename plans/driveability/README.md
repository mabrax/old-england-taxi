...
The original proposal did not authorize implementation. The user subsequently authorized Phases 01, 02 and 03 on 2026-09-07, including reasonable decisions within each phase's scope. Their detailed plans and implementation evidence live in their briefs. Phase 03 implementation proceeds under “Proceed with phase three” without another implementation approval round. Its available-environment verification does not close outstanding iPad/other-engine evidence. Phase 04 remains a planning brief; when it is ready, discuss and agree its implementation/qualification plan and detail that brief. Complete and verify each phase's success criteria and exit condition before beginning the next. Changes to stage scope or artifact dependencies return to that discussion.
