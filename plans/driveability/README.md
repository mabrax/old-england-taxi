...
The [current scene](../../src/lib/scene/validation-scene.ts) renders geometry with OrbitControls and owns its animation loop and disposal. The [viewport](../../src/lib/scene/SceneViewport.svelte) loads a validated artifact, handles cancellation/errors, and selects another zone through navigation. There is no vehicle, simulation, or Rapier dependency yet.
...
1. [Phase 01 — Physical world](./phase-01-physical-world.md)
...
This is a planning proposal only; no implementation phase has started or is authorized by writing it. When each phase is ready, its implementation plan is discussed and agreed with the user, then detailed in that phase brief. Complete and verify its success criteria and exit condition before beginning the next phase. Changes to stage scope or artifact dependencies return to that discussion.
