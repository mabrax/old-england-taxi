...
const output = resolve(process.argv[3] ?? '.zone-cache/phase-01/browser');
...
try {
  ...
  for (const [mode, width, height] of [['desktop', 1440, 900], ['mobile-viewport', 390, 844]]) {
    ...
    for (const zone of catalogue.zones) {
      ...
      run('snapshot', '-i');
      const initial = state();
      assert(initial.canvases === 1 && !initial.overflow && initial.physicsColliders === '6' && initial.physicsSteps === '0', `Initial world failed: ${zone.label}`);
      ...
      assert(hiddenGeometry.physicsColliders === '6' && hiddenGeometry.collisionVisible === 'true' && hiddenGeometry.generatedVisible === 'false', 'QA affected collision inspection');
  ...
  assert(state().physicsColliders === '6', 'Physics depends on generation service');
  ...
  run('network', 'route', `${base}/assets/physical-world-*.js`, '--body', 'throw new Error("Injected physics module failure");');
  run('open', `${base}/?zone=${zone.id}`);
