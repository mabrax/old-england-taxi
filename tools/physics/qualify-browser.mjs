...
async function contextPage(){const context=await browser.createBrowserContext(),page=await context.newPage();await page.setViewport(report.viewport);
 await page.evaluateOnNewDocument(()=>{
  window.__qualification={active:false,step:[],frame:[],raf:[],poses:[],errors:[],focusEvents:[]};
...
try{
 ...
 if(!process.env.FUNCTIONAL_ONLY&&!sustainedZone){const {context,page}=await contextPage();try{
  ...
  for(let cycle=1;cycle<=20;cycle++){
   ...
   for(let r=0;r<3;r++){await click(page,'Drive').catch(()=>click(page,'Resume driving'));await page.keyboard.down('w');await frames(page,45);await page.keyboard.up('w');await click(page,'Reset vehicle');}
   ...
   report.lifecycle.push({cycle,id:zone.id,loadedMs:loaded.elapsedMs,state:s,heap,dom,focusEvents:await page.evaluate(()=>window.__qualification.focusEvents)});save();console.log(engine,'lifecycle',cycle,heap?.used??'memory API unavailable');
  }
  ...
  const fifth=report.lifecycle[4]?.heap?.used,last=report.lifecycle.at(-1)?.heap?.used;report.memoryAcceptance=fifth&&last?{postGcHeap:last<=256*1024**2,growth:last-fifth,allowedGrowth:Math.max(20*1024**2,fifth*.1),growthPass:last-fifth<=Math.max(20*1024**2,fifth*.1),scope:'JS heap only; excludes native/WASM/GPU allocations'}:{result:'unmeasured',reason:'No reliable browser heap API/forced GC'};save();
 }finally{await context.close();}}
}catch(error){report.error=String(error);save();throw error;}finally{save();await browser.close();}
